  ...
  <div class="col-span-1">
    <div class="bg-white rounded border border-gray-200 relative flex flex-col">
        <div class="px-6 pt-6 pb-5 font-bold border-b border-gray-200">
          ...
          <input type="file" multiple @change="upload($event)" id="upload">
          <hr class="my-6" />
          <!-- Progess Bars -->
    ...
    upload($event) {
      ...
      files.forEach((file) => {
        if (file.type === 'audio/mpeg') {
          ...
          // push will return the length of object by subtracting one 
          // we can access to the index of pushed object.
          // because push always pushes items to the end of object
          uploadTask.on('state_changed', (snapshot) => {
            const progress = (snapshot.bytesTransferred / snapshot.totalBytes) * 100;
            this.uploads[uploadIndex].current_progress = progress; 
          }, (error) => { // this callback is for showing errors
            this.uploads[uploadIndex].variant = 'bg-red-400';
            this.uploads[uploadIndex].icon = 'fas fa-times';
            this.uploads[uploadIndex].text_class = 'text-red-400';
            console.log(error);
          }, async () => { // this callback is for completing task
            const song = {
