  ...
  <main>
    ...
    <section class="mb-8 py-20 text-white text-center relative">
      ...
      <img class="relative block mx-auto mt-5 -mb-20 w-auto max-w-full"
        src="assets/img/introduction-music.png" alt="musicImage" />
    </section>
  ...
  methods: {
    ...
    async getSongs() {
      ...
      if (this.songs.length) {
        const lastDoc = await doc(songsCollection, this.songs[0].docID);
        const lastDocSnap = getDoc(lastDoc);
        snapshots = await getDocs(
          songsCollection,
          orderBy('modified_name'), 
          startAfter(lastDocSnap),
          limit(this.maxSongPerPage),
        );
